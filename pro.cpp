...
void retorno_de_opc(pnodo &p,hospital h,cola &C){
	...
	do{
		...
		system("cls");
		menu();
		if(error > 0)cout<<"(valores admitidos del 1 al 5)->";
		scanf("%d",&op);
		error++;
	} while(op	<	1	||	op	>	5);
	switch(op)
...
void eliminar(hospital h)
{
	...
	while (!archivo.eof())
	{
		...
		cout<<"5. FECHA DE NACIMIENTO(DD/MM/AA)---> "<<h.doc.pas.dia<<"/"<<h.doc.pas.mes<<"/"<<h.doc.pas.anio<<endl;
		cout<<"6. DIRECCION-----------> "<<h.doc.pas.direcion<<endl;
		cout<<"7. TELEFONO -----------> "<<h.doc.pas.telefonoCell<<endl;
		cout<<"8. PRIORIDAD-----------> "<<h.doc.pas.prioridad<<endl;
	...
	char respuesta;
	do{
		cout<<"¿ DESEA ELIMINAR PERMANENTEMENTE ESTE ARCHIVO ? ";
		cin>>respuesta;
		cout<<endl;
...
void modificar(hospital &h)
{
	...
	switch(opc)
	{
		// AQUI DEBE IR UN MODIFICA DNI PS CAUSAA, Y CUANDO LO MODIFICQUE QUE TODO EL CONTENIDO SE COPIE EN UN NUEVO ARCHIVO Y LUEGO QUE REEMPLACE AL ANTERIOR  MANYAS MANO ?
	case 2:
	{
		do{
			cout<<"\t\tNOMBRE: ";
...
void encolaPrioridad(cola &c,hospital h)
{
	...
	if(c.primero==NULL)
	{
	   ...
	   if(h.doc.pas.prioridad<c.primero->info.doc.pas.prioridad)
	{
	   ...
	   else
	   {
		   ...
		   while(p!=NULL && p->info.doc.pas.prioridad<=h.doc.pas.prioridad)
		   {
			   ant=p;
			   p=p->sgte;
		   }
...
void imprime(cola C)
{
	...
	while(q!=NULL)
	{
		cout<<q->info.doc.pas.prioridad<<endl;
		q=q->sgte;
	}
}
